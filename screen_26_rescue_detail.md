...
**Upload:** each photo via `SignUploadBatch (BV)` `{ items: [{ purpose: "RESCUE_PHOTO", ... }] }` (`screen_4`) → use `list[0].mediaId` (canonical — petapp-be#906 shipped; xem `screen_4`); no AI scan. ⏳ GAP petapp-be#908 — `RESCUE_PHOTO` chưa có trong `MediaPurposeGQL` (các giá trị hiện tại: AVATAR, COVER_PHOTO, POST_PHOTO, POST_VIDEO, PRODUCT, HEALTH_DOC, KYC_DOC, LOST_PET).
